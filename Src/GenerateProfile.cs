        ...
        private HttpResponse generateProfile(HttpRequest req)
        {
            HttpResponse generateDefExp(HttpRequest rq, int operation, string name, Func<KtaneModuleInfo, KtaneModuleDifficulty, bool> filter)
            {
                ...
                return HttpResponse.Create(
                    new JsonDict { { "DisabledList", _config.Current.KtaneModules.Where(k => k.ModuleID != null && (k.Type == KtaneModuleType.Regular || k.Type == KtaneModuleType.Needy) && filter(k, desired)).Select(k => k.ModuleID).ToJsonList() }, { "Operation", operation } }.ToString(),
