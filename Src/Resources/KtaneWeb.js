...
$(function() {
	var filter = {};
	try { filter = JSON.parse(lStorage.getItem('filters') || '{}') || {}; } catch (exc) {}
	var selectable = lStorage.getItem('selectable') || 'manual';
	if (Ktane.Selectables.indexOf(selectable) === -1)
		selectable = 'manual';
	var preferredManuals = {};
	try { preferredManuals = JSON.parse(lStorage.getItem('preferredManuals') || '{}') || {}; } catch (exc) {}

	function compare(a, b) {
		return ((a < b) ? -1 : ((a > b) ? 1 : 0)); }
	var sorts = {
		'name': { fnc: function(elem) {
			return $(elem).data('sortkey'); }, bodyCss: 'sort-name', radioButton: '#sort-name' },
		'defdiff': { fnc: function(elem) {
			return Ktane.Filters[2].values.indexOf($(elem).data('defdiff')); }, bodyCss: 'sort-defdiff', radioButton: '#sort-defuser-difficulty' },
		'expdiff': { fnc: function(elem) {
			return Ktane.Filters[3].values.indexOf($(elem).data('expdiff')); }, bodyCss: 'sort-expdiff', radioButton: '#sort-expert-difficulty' }
	};
	...
	$('#sort-by-name').click(function() { setSort('name');
		return false; });
	$('#sort-by-difficulty').click(function() { setSort(sort === 'defdiff' ? 'expdiff' : sort === 'expdiff' ? 'name' : 'defdiff');
		return false; });

    // Radio buttons (visible only on mobile UI)
	$('#sort-name').click(function() { setSort('name');
		return true; });
	$('#sort-defuser-difficulty').click(function() { setSort('defdiff');
		return true; });
	$('#sort-expert-difficulty').click(function() { setSort('expdiff');
		return true; });

	$("#theme-light").click(function() { setTheme("light");
		return true; });
	$("#theme-dark").click(function() { setTheme("dark");
		return true; });
...
var theme = lStorage.getItem("theme") || "light";
