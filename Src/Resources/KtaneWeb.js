...
function initializePage(modules, initIcons, initDocDirs, initDisplays, initFilters, initSelectables, souvenirAttributes, iconSpriteMd5) {
        ...
        switch (newView) {
            case 'List': {
                ...
                for (var i = 0; i < modules.length; i++) {
                    ...
                    if (mod.TwitchPlays) {
                        ...
                        infos.append(el("div", "inf-twitch inf inf2", { title: mod.TwitchPlaysInfo }, mod.TwitchPlays.ScorePerModule || mod.TwitchPlays.ScoreExplanation ? 'S' : mod.TwitchPlays.Score));
                    }
            ...
            case 'PeriodicTable': {
                ...
                for (let i = 0; i < modules.length; i++) {
                    ...
                    let a = el('a', `module ${mod.ExpertDifficulty} compatibility-${mod.Compatibility}`, el('div', `symbol ${mod.DefuserDifficulty}`, mod.Symbol || '??', el('div', 'icon', { style: `background-image:url(iconsprite/${iconSpriteMd5});background-position:-${mod.X * 32}px -${mod.Y * 32}px` })), el('div', 'name', el('div', 'inner', mod.Name)), el('div', 'tpscore', mod.TwitchPlays ? mod.TwitchPlays.Score : ''), el('div', 'souvenir', souvenirStatuses[(mod.Souvenir && mod.Souvenir.Status) || 'Unexamined']), manualSelector);
    ...
    function makeClickHander(lnk, isMobileOpt, mod) {
        return function () {
            ...
            menuDiv.appendChild(el('div', 'bottom-links', el('div', null, el('a', null, { href: `find-log?find=${encodeURIComponent(mod.ModuleID)}` }, 'Find example logfile')), el('div', null, el('a', null, { href: '#', onclick: function () { setEditUi(mod); popup($(lnk), $('#module-ui')); return false; } }, `Edit this ${mod.Type === 'Widget' ? 'widget' : 'module'}`))));
            if (!isMobileOpt)
    ...
    $('input#profile-file').change(function () {
        const files = document.getElementById('profile-file').files; if (files.length === 1) {
            setProfile(files[0]);
        }
    });
